...
export default function Chat() {
    ...
    const { data: chat, isLoading, error, } = useQuery(trpc.chat.byId.queryOptions(chatId ? { chatId } : skipToken));
    if (isLoading) {
    ...
    return (<div>
      ...
      <div className="messages-container">
        {messages &&
            messages.length > 0 &&
            messages.map((message, index) => (<MessageBubble key={message.id} message={message} isCurrentUser={(user === null || user === void 0 ? void 0 : user.id) === message.sender.id}/>))}
      </div>
    </div>);
}
